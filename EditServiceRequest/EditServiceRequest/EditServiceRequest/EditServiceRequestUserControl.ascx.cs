...
namespace EditServiceRequest.EditServiceRequest
{
    public partial class EditServiceRequestUserControl : UserControl
    {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    SPSecurity.RunWithElevatedPrivileges(delegate()
                    {
                        ...
                        if (list != null)
                        {
                            ...
                            //var collection = web.Lists.TryGetList("Service Requests").GetItems();
                            foreach (SPListItem item in list.Items)
                            {
                                if (item["Service Request ID"] != null)
                                {
                                    string value = item["Service Request ID"].ToString();

                                    //var listItem = new ListItem(item["Service Request ID"].ToString());
                                    ddlSerReqID.Items.Add(value);
                                }
                ...
                using (SPSite site = new SPSite("http://sp2010/"))
                {
                    ...
                    if (collection.Count > 0)
                    {
                        foreach (SPListItem item in collection)
                        {
                            ...
                            //ddlkeyword.SelectedItem.Text = item["Keywords"].ToString();
                            //txtkeyword.Text = item["Keywords"].ToString();
                            string Value = item["Keywords"].ToString();
                            //value = Value.Substring(Value.IndexOf('#') + 1);
                            ddlkeyword.SelectedItem.Text = Value.Substring(Value.IndexOf('#') + 1);
